#ifndef MY_UNORDERED_MAP_hpp
#define MY_UNORDERED_MAP_hpp
...
public:
    ...
    MyUnorderedMap(const mumap& map): MyUnorderedMap(){
        ...
        try{
            ...
            // I can little faster, but it would be copy-past
    ...
    /**
     ...
     @exception std::bad_alloc();
     */
    MyUnorderedMap(mumap&& map) noexcept : __size(map.__size), __count(map.__count), array(map.array),
    ...
    /**
     ...
     @exception std::bad_alloc();
     */
    mumap& operator=(mumap&& map) noexcept {
        if (&map == this) return *this;
    ...
    void rehash(size_t new_size){
        if (new_size * __max_load_factor < __count)
            throw std::out_of_range("unordered_map::rehash: index is less then the minimum possible");
        __rehash(new_size);
    }
    ...
    std::pair<iterator, bool> insert(const item& pair){
        if (__size * __max_load_factor < __count + 1)
            ...
            size_t(std::ceil(float(__count + 1) / __max_load_factor))));
